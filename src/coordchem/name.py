...
from coordchem.parser import METALS
...
def metal_data(name: str)->ParsedComplex:
        ...
        for symbols, metal_names in metals_by_longest_name:
            ...
            if cation_name in _normalize_name:
                 return symbols
            elif anion_name in _normalize_name:
                return symbols
...
#cation_name=metal_names[0]
  #          anion_name=metal_names[1]   
   #         if cation_name in name:
    #            if len(cation_name)>best_length:
     #               best_match=symbols
      #              best_length=len(cation_name)
       #         return best_match
        #    elif anion_name in name:
         #       if len(anion_name)>best_length:
          #          best_match=symbols
           #         best_length=len(anion_name)
            #    return best_match
